...
void ESPL::from_prior(DNest4::RNG& rng)
{
    ...
    // Prior for Pb, U(0., 1.)
    DNest4::Uniform uniform_Pb(0., 1.);
    rho = uniform_Pb.generate(rng); 
    
    // Prior for mu_b U(0., 1.)
    DNest4::Uniform uniform_mu_b(0., 1.);
    rho = uniform_mu_b.generate(rng); 

    // Prior for sig_b Exp(0.5)
    DNest4::Exponential exponential_sig_b(0.5);
    rho = exponential_sig_b.generate(rng); 
...
double ESPL::perturb(DNest4::RNG& rng)
{
    ...
    switch(param) {
        ...
        case 6:{
            DNest4::Uniform uniform_Pb(0., 1.);
            log_H += uniform_Pb.perturb(Pb, rng); 
            break;
        }
        case 7:{
            DNest4::Uniform uniform_mu_b(0., 1.);
            log_H += uniform_mu_b.perturb(mu_b, rng); 
            break;
        }
        case 8:{
            log_H += (1/0.5)*sig_b;
            DNest4::Exponential exponential_sig_b(0.5);
            log_H += exponential_sig_b.perturb(sig_b, rng); 
            break;
        }
    }
...
double ESPL::log_likelihood() const
{
    ...
    // Gaussian mixture sampling distribution 
    double log_L = 0.0;

    for (size_t i = 0; i < F.size(); i++){
        log_L += log(
            Pb*exp(-0.5*pow((F[i] - mu[i])/sigF[i], 2.))/sqrt(2*M_PI*sigF[i]*sigF[i]) 
            + (1. - Pb)*exp(-0.5*pow((F[i] - mu_b)/sig_b, 2.))/sqrt(2*M_PI*sig_b*sig_b) 
            );
    }
...
{
	out<<DeltaF<<' '<<Fb<<' '<<t0<<' '<<u0<<' '<<tE<<' '<<rho<<' '<<Pb<<' '<<mu_b<<' '<<sig_b;
}

string ESPL::description() const
{
	return string("DeltaF, Fb, t0, u0, tE, rho, Pb, mu_b, sig_b");
}
