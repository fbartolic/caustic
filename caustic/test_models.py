...
# Load event data
data_dir = '/home/fran/data/OGLE_ews/2017/blg-0989'
...
fig, ax = plt.subplots(figsize=(25, 10))
plot_prior_model_samples(ax, event, FiniteSourcePointLens(event, 
    errorbar_rescaling='additive_variance'), 10)
plt.show()

#with PointSourcePointLens(event, 
#    errorbar_rescaling='additive_variance') as model1:
#    model1.sample(output_dir1, n_tune=500, n_samples=200)
...
#with PointSourcePointLensAnnualParallax(event) as model3:
#    model3.sample(output_dir3, n_tune=2000, n_samples=2000)
...
#       'gamma_lambda_lowerbound____0_0']
#
...
plot_model_and_residuals(ax, event, 
            FiniteSourcePointLens(event, errorbar_rescaling='additive_variance'),
